<script lang="ts">
    ...
    let selectedFontObject = text_fonts.find((font) => font.value == selectedTextStyle.fontFamily) || text_fonts[0];
	$: selectedTextStyle.fontFamily, selectedFontObject = text_fonts.find((font) => font.value == selectedTextStyle.fontFamily) || text_fonts[0];
</script>
...
<Popover.Trigger asChild let:builder>
    ...
    role="combobox"
    aria-expanded={open}
    class="w-[120px] h-[2rem] justify-between rounded-r-none mr-0 pr-[6px] gap-y-1"
    >
...
<Popover.Content class="w-[200px] p-0">
    ...
    <Command.Group>
        ...
        >
            ...
            class={cn(
                "mr-2 h-4 w-4",
                value !== font.value && "text-transparent"
            )}
            />
            {font.label}
        </Command.Item>
